...
describe('NotificationScheduleService.getItemsDueForNotification', () => {
  it('returns an empty array when the input is empty', () => {
    expect(svc.getItemsDueForNotification([], 3, REF)).toEqual([])
  })
  ...
  it('returns only items within the notification window', () => {
    ...
    const due = svc.getItemsDueForNotification(items, 3, REF)
  ...
  it('excludes items with null expiry_date', () => {
    ...
    expect(svc.getItemsDueForNotification(items, 3, REF)).toHaveLength(0)
  })
  ...
  it('respects a wider defaultDaysBefore', () => {
    ...
    const due3 = svc.getItemsDueForNotification(items, 3, REF)
    const due7 = svc.getItemsDueForNotification(items, 7, REF)
  ...
  it('includes expiry-today items', () => {
    ...
    expect(svc.getItemsDueForNotification(items, 3, REF)).toHaveLength(1)
  })
